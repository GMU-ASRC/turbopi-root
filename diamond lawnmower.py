...
class TrackingState(StateMachine):
    ...
    def lawnmower_search(self):
        start_time = time.time()
        while time.time() - start_time < 30:
            if self.robot.foe_detected():
                print("Foe detected")
                self.robot.mode = 'tracking'
                return
            

			#lawnmowering
            self.robot.move(50, 90, 0) 
            time.sleep(3)  
            self.robot.move(0, 90, 1)
            time.sleep(1.5) 
        self.robot.mode = 'idle'

    def detect_wall_and_initiate_lawnmower(self, distance):
        if distance < self.stuck_distance:
            # measurement 1
            measurement_1 = self.robot.sonar.getDistance()
            self.robot.move(0, 90, 1)  # Pivot
            time.sleep(0.5)  # rotation time
            # measurement 2
            measurement_2 = self.robot.sonar.getDistance()
            
            # comparison
            if max(measurement_1, measurement_2) > min(measurement_1, measurement_2) * 1.65:
                self.lawnmower_search()  # Start lawnmower search	
...
class SandmanProgram(camera_binary_program.CameraBinaryProgram):
    ...
    def control(self):
        ...
        if self.stuck.current_state == self.stuck and self.robot.mode != 'tracking':
            self.stuck.detect_wall_and_initiate_lawnmower(self.distance)
